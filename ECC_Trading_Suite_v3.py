...
def api_mode():
    ...
    with LOCK:
        STATE["mode"] = mode
        STATE["watchlist"] = (["RELIANCE.NS","TCS.NS","INFY.NS","HDFCBANK.NS","NSEI","BANKNIFTY.NS"]
...
<title>ECC Trading Suite v3</title>
<style>
...
.bnav-btn{flex:1;padding:10px 4px;border:none;background:transparent;color:#6e7681;font-size:12px;font-weight:600;min-height:62px}
...
<header class="hdr">
  <div><div class="brand">ECC Trading v3</div><div style="font-size:11px;color:#6e7681">Personal paper trading</div></div>
...
<div id="tab-trade" class="tab active">
  <div class="card">
    ...
    <button class="btn btn-b" style="width:100%" onclick="doSearch()">Search &amp; Add to Watchlist</button>
  ...
  <div class="card">
    ...
    <div class="chart-tog">
      <button class="btn btn-o" onclick="setChartType('line')">Line</button>
      <button class="btn btn-o" onclick="setChartType('candle')">Candle</button>
      <button class="btn btn-o" onclick="setChartType('bar')">Bar</button>
    </div>
    <canvas id="priceChart" style="height:220px"></canvas>
  </div>
...
<div id="tab-strat" class="tab">
  ...
  <div id="customBox" class="card" style="display:none">
    ...
    <div style="font-size:13px;color:#6e7681;margin-bottom:14px">
      ...
      <code>buy when 5 sma crosses above 20 sma</code> · <code>buy when bollinger lower band touched</code>
    </div>
  ...
  <div class="card" style="display:flex;justify-content:space-between;align-items:center">
    <label style="display:flex;gap:8px;align-items:center;font-size:15px"><input type="checkbox" id="stratActive" style="width:20px;height:20px"> Strategy Active (auto-trades)</label>
...
<div id="tab-scan" class="tab">
  <div class="card">
    ...
    <input id="chartinkSlug" class="inp" placeholder="Paste your Chartink screener URL, e.g. https://chartink.com/screener/my-scan">
    <button class="btn btn-b" style="width:100%" onclick="pullChartink()">Pull Symbols into Watchlist</button>
    <div style="font-size:12px;color:#6e7681;margin-top:8px">Chartink has no official API - this uses their public screener endpoint and may occasionally need a fix if they change their site.</div>
...
<div id="tab-bt" class="tab">
  <div class="card">
    ...
    <input id="btAsset" class="inp" placeholder="Any symbol e.g. RELIANCE.NS, BTC-USD, TCS.NS" value="RELIANCE.NS">
  ...
  <div id="btResults" style="display:none">
    ...
    <div class="card"><canvas id="btPriceC" style="height:140px"></canvas></div>
    <div class="card"><canvas id="btPortC" style="height:100px"></canvas></div>
  </div>
</div>

<div id="tab-help" class="tab">
  <div class="info-box"><div class="info-title">Mobile notifications</div>
  <div class="info-text">Android/Chrome: tap Allow below, then notifications work directly.<br>
  iPhone/Safari: you MUST tap Share &rarr; Add to Home Screen first - Apple blocks background notifications in regular Safari tabs. This is an Apple restriction, not fixable from any website.</div>
  <button class="btn btn-b" style="margin-top:10px" onclick="askNotif()">Allow Notifications</button></div>

  <div class="info-box"><div class="info-title">Sensibull</div>
  <div class="info-text">Sensibull does not offer a public or demo API for third-party apps, so direct integration is not possible.
  This site simulates options-style paper trading internally instead, giving you the same practice experience without needing Sensibull.</div></div>

  <div class="info-box"><div class="info-title">Keep it running 24/7 free</div>
  <div class="info-text">Use UptimeRobot (free) to ping <code>/health</code> every 5 minutes so Render's free tier doesn't sleep.</div></div>

  <div class="info-box"><div class="info-title">Install as an app</div>
  <div class="info-text">Android: menu &rarr; Add to Home screen. iPhone: Share &rarr; Add to Home Screen.</div></div>
</div>
...
<nav class="bnav">
  ...
  <button class="bnav-btn" onclick="showTab('scan',this)">Scanner</button>
  <button class="bnav-btn" onclick="showTab('bt',this)">Backtest</button>
  <button class="bnav-btn" onclick="showTab('help',this)">Help</button>
</nav>

<script>
let tickTimer=null, selAsset='BTC-USD', currentSide='BUY', lastState=null, chartType='line', currentMode='GLOBAL';
...
async function doSearch(){
  ...
  try{
    ...
    toast('Added '+d.info.symbol+' to watchlist');
    refresh();
...
function drawChart(id,series,type='line'){
  ...
  const dpr=window.devicePixelRatio||1,cw=c.clientWidth||300,ch=c.clientHeight||150;
  ...
  const lp=48,rp=6,tp=6,bp=6,pw=cw-lp-rp,ph=ch-tp-bp;
  ctx.strokeStyle='#21262d';ctx.fillStyle='#6e7681';ctx.font='10px ui-monospace,monospace';ctx.lineWidth=0.5;
  for(let i=0;i<=3;i++){const y=tp+ph*i/3,val=mx-(mx-mn)*i/3;ctx.beginPath();ctx.moveTo(lp,y);ctx.lineTo(cw-rp,y);ctx.stroke();ctx.fillText(fmv(val),2,y+3);}
  const n=series.length;
  const xAt=i=> lp + pw*(i/(n-1||1));
  ...
  if(type==='line'){
    ...
    series.forEach((pt,i)=>{
      ...
      const yo=yAt(o),yc=yAt(cl);ctx.fillRect(x-bw/2,Math.min(yo,yc),bw,Math.max(1,Math.abs(yo-yc)));
    });
  } else if(type==='bar'){
    const bw=Math.max(2,pw/n*0.6);
...
function askNotif(){
  ...
  Notification.requestPermission().then(p=>{
    if(p==='granted'){toast('Notifications enabled');
      ...
      new Notification('ECC Trading',{body:'Notifications are ON.'});
...
function connectSSE(){
  ...
  es.onmessage=e=>{const a=JSON.parse(e.data);fireNotif(a);addAlert(a);toast(a.asset+' '+a.pattern);};
...
function addAlert(a){
  ...
  div.innerHTML=`<b>${a.asset}</b> - ${a.pattern} <span style="color:${bull?'#3fb950':'#f85149'}">${a.direction}</span><br><span style="color:#6e7681;font-size:12px">${a.time} @ ${a.price}</span>`;
...
function updateAssetSelect(){
  ...
  if(assets.includes(cur))sel.value=cur; else if(assets.length)selAsset=assets[0];
}
...
async function finalizeStrat(){
  ...
  try{
    ...
    const d=await api('/api/strategy','POST',body);lastState=d.state;render(d.state);toast('Strategy saved');
  }catch(e){toast(e.message,'err');}
}
updateStratUI();
